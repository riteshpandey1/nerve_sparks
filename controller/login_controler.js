...
const { userDB, dealershipDB } = require("../config/dbConfig");
...
const authUser = async (req, res, next) => {
  try {
    ...
    if (!user)
      return res.status(400).send({ message: "User not found!", staus: 404 });
...
const authDealer = async (req, res, next) => {
  try {
    ...
    if (!dealer)
      return res.status(400).send({ message: "Dealer not found!", staus: 404 });
